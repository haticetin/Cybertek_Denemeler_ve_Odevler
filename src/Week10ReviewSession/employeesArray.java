...
public class employeesArray {
	...
	// print all job titles 
		...
		for(String value : employees) {
			...
			String job = value.substring(value.indexOf("[")+1, value.indexOf("]"));
			System.out.println(job);
		}
	
	
	// print all firtname-lastname jobtitle
		
		for(String values : employees) {
			
			String nameSurname = values.substring(0, values.lastIndexOf(" ["));
			System.out.print(nameSurname + " - ");
			String job = values.substring(values.indexOf("[")+1, values.indexOf("]"));
			System.out.println(job);
			System.out.println();
		}
	
//		Tracy Bartle should be Geological Engineer
//		if true print pass: Verification successful
//		else print: Verification Error
		...
		for(String values : employees) {
			...
			String job = values.substring(values.indexOf("[")+1, values.indexOf("]"));
			System.out.println(job);
			if(nameSurname.equalsIgnoreCase("Tracy Bartle")) {
